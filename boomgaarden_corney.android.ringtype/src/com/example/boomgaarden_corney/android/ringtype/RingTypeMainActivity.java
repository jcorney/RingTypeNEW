...
public class RingTypeMainActivity extends Activity {
	...
	private AudioManager mRingType;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_ring_type_main);
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		getMenuInflater().inflate(R.menu.ring_type_main, menu);
		return true;
	}
	...
	private String sendHttpRequest(String myURL, String postParameters)
		...
		} else if (postParameters.equals("RINGTYPE")) {
			writer.write(buildPostRequest(paramsRingType));
			paramsRingType = new ArrayList<NameValuePair>();
	...
	private void setRingTypeData() {
		ringType = mRingType.getMode();
		
		switch (ringType) {
		case 0:
			ringTypeStr = ("Mode " +  String.valueOf(ringType) + " - Normal audio mode: not ringing and no call established.");
			break;
			
		case 1:
			ringTypeStr = ("Mode " +  String.valueOf(ringType) + " - Ringing audio mode. An incoming is being signaled.");
			break;
		
		case 2:
			ringTypeStr = ("Mode " +  String.valueOf(ringType) + " - In call audio mode. A telephony call is established.");
			break;
		
		case 3:
			ringTypeStr = ("Mode " +  String.valueOf(ringType) + " - In communication audio mode. An audio/video chat or VoIP call is established.");
			break;
		...
		paramsRingType.add(new BasicNameValuePair("Ring Type",ringTypeStr));
	}
	...
	private void showRingTypeData() {
		StringBuilder results = new StringBuilder();

		results.append("Ring Type: "
				+ ringTypeStr + "\n");
	...
	private void sendRingTypeData() {
		...
		if (networkInfo != null && networkInfo.isConnected()) {
			// Send HTTP POST request to server which will include POST
			// parameters with Accelerometer info
			new SendHttpRequestTask().execute(SERVER_URL, "RINGTYPE");
		} else {
			setErrorMsg("No Network Connectivity");
			showErrorMsg();
		}
